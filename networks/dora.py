...
def get_block_dims_and_alphas(
    ...
    num_total_blocks = DoRANetwork.NUM_OF_BLOCKS * 2 + 1
...
def get_block_lr_weight(
    ...
    max_len = DoRANetwork.NUM_OF_BLOCKS  # フルモデル相当でのup,downの層の数
...
def remove_block_dims_and_alphas(
    ...
    if mid_lr_weight != None:
        if mid_lr_weight == 0:
            block_dims[DoRANetwork.NUM_OF_BLOCKS] = 0
            if conv_block_dims is not None:
                conv_block_dims[DoRANetwork.NUM_OF_BLOCKS] = 0
    if up_lr_weight != None:
        for i, lr in enumerate(up_lr_weight):
            if lr == 0:
                block_dims[DoRANetwork.NUM_OF_BLOCKS + 1 + i] = 0
                if conv_block_dims is not None:
                    conv_block_dims[DoRANetwork.NUM_OF_BLOCKS + 1 + i] = 0
...
def get_block_index(lora_name: str) -> int:
    ...
    if m:
        ...
        if g[0] == "down":
            ...
            block_idx = DoRANetwork.NUM_OF_BLOCKS + 1 + idx

    elif "mid_block_" in lora_name:
        block_idx = DoRANetwork.NUM_OF_BLOCKS  # idx=12
...
class DoRANetwork(torch.nn.Module):
    ...
    def __init__(
            ...
            module_class: Type[object] = DoRAModule,
        ...
        for i, text_encoder in enumerate(text_encoders):
            ...
            text_encoder_loras, skipped = create_modules(False, index, text_encoder,
                                                         DoRANetwork.TEXT_ENCODER_TARGET_REPLACE_MODULE)
            self.text_encoder_loras.extend(text_encoder_loras)
            skipped_te += skipped
        logger.info(f"create LoRA for Text Encoder: {len(self.text_encoder_loras)} modules.")

        # extend U-Net target modules if conv2d 3x3 is enabled, or load from weights
        target_modules = DoRANetwork.UNET_TARGET_REPLACE_MODULE
        if modules_dim is not None or self.conv_lora_dim is not None or conv_block_dims is not None:
            target_modules += DoRANetwork.UNET_TARGET_REPLACE_MODULE_CONV2D_3X3
    ...
    def merge_to(self, text_encoder, unet, weights_sd, dtype, device):
        ...
        for key in weights_sd.keys():
            if key.startswith(DoRANetwork.LORA_PREFIX_TEXT_ENCODER):
                apply_text_encoder = True
            elif key.startswith(DoRANetwork.LORA_PREFIX_UNET):
                apply_unet = True
    ...
    def get_lr_weight(self, lora: DoRAModule) -> float:
        lr_weight = 1.0
        block_idx = get_block_index(lora.lora_name)
        if block_idx < 0:
            return lr_weight

        if block_idx < DoRANetwork.NUM_OF_BLOCKS:
            if self.down_lr_weight != None:
                lr_weight = self.down_lr_weight[block_idx]
        elif block_idx == DoRANetwork.NUM_OF_BLOCKS:
            if self.mid_lr_weight != None:
                lr_weight = self.mid_lr_weight
        elif block_idx > DoRANetwork.NUM_OF_BLOCKS:
            if self.up_lr_weight != None:
                lr_weight = self.up_lr_weight[block_idx - DoRANetwork.NUM_OF_BLOCKS - 1]
    ...
    def backup_weights(self):
        # 重みのバックアップを行う
        loras: List[DoRAInfModule] = self.text_encoder_loras + self.unet_loras
        for lora in loras:
